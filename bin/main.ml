...
let jouer_coup (plateau, j) (x, y) =
  ...
  List.iter
    (fun dir ->
      if faut_retourner (plateau, j) (x, y) dir then
        retourner (plateau, j) (x, y) dir)
    liste_directions
...
let () =
  print_int (partie (Minmax1.strategie_minmax_ab 6) strategie_aleatoire true)
